...
export async function handleInbound({ shop, from, body, mediaUrl, mediaType }) {
  ...
  // The prompt tells L1 to stop asking once the customer declines, but a model
  // cannot write state — only code can. Without this the conversation asks for
  // the photo forever, which is exactly what the test bench caught.
  ...
  if (state.asset_capture === 'pending' && state.turns >= MAX_TURNS) {
