...
// ── L1c — Scope veto ─────────────────────────────────────────
// Only runs when the deterministic check in referral.js has already decided a
// job is outside the shop's trade. The model's only power here is to say "no,
// keep it" — it cannot create a referral. That asymmetry is the whole point:
// wrongly sending a customer away is far worse than wrongly keeping them, so
// the expensive-to-reverse direction stays in code and the model only vetoes.
const SCOPE_SCHEMA = {
  type: 'object',
  additionalProperties: false,
...
export async function confirmOutOfTrade({
  complaint, shopName, tradeLabel, equipmentTypes, boundaries, suspected, ctx,
}) {
  const system = `You decide one thing: whether a job a customer has described is work ${shopName} does.

${shopName} is a ${tradeLabel} company. They work on: ${(equipmentTypes || []).join(', ')}.
${boundaries ? `In the owner's own words, what they do NOT take on: "${boundaries}"` : ''}

Another check has already flagged this as sounding like ${suspected} work rather than ${tradeLabel}. Your job is to catch that check being wrong.

Answer in_scope TRUE if there is any reasonable reading where this shop would take the job — the equipment is on their list, the trades overlap on this kind of work, or the message is too vague to tell what the equipment even is.
Answer in_scope FALSE only when it is plainly a different trade's work AND nothing in the owner's own boundaries suggests otherwise.

When you are unsure at all, answer TRUE. Turning away a customer who could have been served is the costly mistake here.`;

  try {
    const text = await call({
      model: MODELS.triage,
      system,
      messages: [{ role: 'user', content: `CUSTOMER SAID: "${complaint}"` }],
      max_tokens: 200,
      kind: 'l1c_scope',
      ctx,
      schema: SCOPE_SCHEMA,
    ...
    // No verdict means no referral: the caller treats null as "keep it".
    console.error('[fp] scope check failed, keeping the job', e.message);
    return null;
  }
}
