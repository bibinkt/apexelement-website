...
export async function sendSms(from, to, body) {
  // FP_TEST_SMS=1 captures instead of sending, so the harness can run the real
  // chain with no telephony and no A2P approval. Production never sets it.
  const box = sandbox.getStore();
  if (box) {
    box.sent = box.sent || [];
    box.sent.push({ from, to, body, at: new Date().toISOString() });
    return { ok: true, sid: `SMtest${box.sent.length}` };
  }
  if (process.env.FP_TEST_SMS === '1') {
    globalThis.__FP_SENT = globalThis.__FP_SENT || [];
    globalThis.__FP_SENT.push({ from, to, body });
    return { ok: true, sid: `SMtest${globalThis.__FP_SENT.length}` };
  }
  const r = await fetch(
...
export async function fetchMedia(url) {
  // A data: URL lets the test page feed a real photo through the identical
  // vision path — same fetchMedia, same l2Vision, same confidence router.
  if (String(url).startsWith('data:')) {
    const [head, b64] = String(url).split(',');
    return { base64: b64, type: (head.match(/data:([^;]+)/) || [])[1] || 'image/jpeg' };
  }
...
/** Line type, so we hand out the right call-forwarding code. */
