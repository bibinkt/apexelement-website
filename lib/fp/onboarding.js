...
/**
 * Someone phoned the marketing line. Send exactly ONE message asking permission
 * before anything else — nothing further happens unless they reply YES.
 *
 * Gated on FP_CALL_CONSENT so the code can ship before the campaign declares
 * this opt-in path; sending outside a declared path is what got us rejected
 * three times.
 */
export async function requestConsentAfterCall({ marketingNumber, from }) {
  if (process.env.FP_CALL_CONSENT !== '1') return { action: 'disabled' };
  ...
  // Never ask twice, and never interrupt someone already part-way through.
  if (shop && shop.onboarding_step !== 'consent_declined') return { action: 'already_known' };
  ...
  if (!shop) {
    shop = await insert('fp_shops', {
      ...
      onboarding_data: { consent_requested_at: new Date().toISOString(), source: 'inbound_call' },
    });
  } else {
    await update('fp_shops', `id=eq.${shop.id}`, { onboarding_step: 'awaiting_consent' });
  }

  await say(marketingNumber, from, CALL_CONSENT_TEXT);
