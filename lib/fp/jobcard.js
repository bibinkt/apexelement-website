 ...
 */
export function routeConfidence(extract, threshold = CONFIDENCE_THRESHOLD) {
  const fields = {};
  if (!extract) {
  ...
  for (const f of FIELDS) {
    ...
    if (value.toLowerCase() === 'unknown' || conf < threshold) {
 ...
 */
export function assembleCard({ shop, conversation, fields, complaint, question, answer, mediaUrl, address, duration, stillRunning, availability }) {
  const f = fields || {};
  ...
  const lines = [
    ...
    '',
    'THE STORY',
    `  Going on for   ${duration || 'not given'}`,
    `  Running?       ${stillRunning || 'not given'}`,
    `  Free when      ${availability || 'not given'}`,
  ];
